...
void BLESerial::begin(const char *name) {
  ...
  // Use level 0 so esp_ble_set_encryption is not auto-called on connect (BLEDevice.cpp).
  // ESP-IDF 5+ may define ESP_BLE_SEC_NONE; Arduino-ESP32 2.0.x does not — use (esp_ble_sec_act_t)0.
  #if defined(ESP_BLE_SEC_NONE)
