...
void setup() {
  ...
  validate_status();
      ...
      // 160 MHz idle power; re-lock UART baud — changing CPU freq skews HW Serial unless updated.
...
inline void kiss_write_packet() {
  ...
  host_write_len = 0;
    ...
    #endif
    #if HAS_WIFI
      if (wifi_host_is_connected()) { wifi_remote_flush(); }
    #endif
  #endif
}
...
bool startRadio() {
  ...
  if (!radio_online && !console_active) {
    // Debug: Print state for Station G2 troubleshooting
    #if BOARD_MODEL == BOARD_STATION_G2
        ...
        if (eeprom_checksum_valid()) {
          ...
          if (modem_installed) {
              ...
              if (device_init()) {
                ...
                hw_ready = false;
                #if BOARD_MODEL == BOARD_STATION_G2
                  Serial.println("DEBUG: device_init() FAILED, hw_ready=false");
                #endif
              }
            #else
              hw_ready = true;
