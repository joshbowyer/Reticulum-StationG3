...
sx126x::sx126x() :
  ...
  _spiSettings(10E6, MSBFIRST, SPI_MODE0),  // 10 MHz for Station G2 PCB signal integrity per spec
  #else
...
bool sx126x::preInit() {
  ...
  #if BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_HELTEC32_V4 || BOARD_MODEL == BOARD_TDECK || BOARD_MODEL == BOARD_XIAO_S3 || BOARD_MODEL == BOARD_STATION_G2 || BOARD_MODEL == BOARD_STATION_G3
...
int sx126x::begin(long frequency) {
  ...
  setPacketParams(_preambleLength, _implicitHeaderMode, _payloadLength, _crcMode);
      ...
      digitalWrite(pin_lna_en, LOW); // LNA on (active-low), default RX-ready state
    #endif
  #endif
...
int sx126x::endPacket() {
  ...
  #if HAS_LORA_LNA
    #if LORA_LNA_STATIONG3
      // endPacket() waits synchronously for TX_DONE, so restore the RX-ready LNA state here.
      digitalWrite(pin_lna_en, LOW); // LNA back on after TX
    #endif
  #endif
